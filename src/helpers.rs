...
// Structs ──────────────────────────────────────────────────────────────────────────────
use crate::structs::{App, ThemeMappingType};
    ...
    T: for<'a> Deserialize<'a>
{
    match File::open(Path::new(path)) {
        ...
        Ok(file) => match serde_json::from_reader(file) {
            Err(error) => panic!(
                "❌ Could not convert [{}] to JSON because [{}]",
                path, error
            ),
            Ok(json) => json
        },
...
pub fn set_apps_theme(theme_name: String, app: &App) {
    for path in app.config.paths.clone().unwrap() {
        let file_content: String = fs::read_to_string(&path).unwrap_or_else(|error| {
            panic!(
        ...
        fs::write(&path, content.into_owned()).unwrap_or_else(|error| {
            panic!(
